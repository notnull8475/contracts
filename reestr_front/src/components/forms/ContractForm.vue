  ...
  <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="600">
    <v-card>
      ...
      <v-card-text>
        ...
        <!-- ID организации -->
        <v-text-field v-model="form.organization_id" label="ID организации" />

        <!-- Тип валидности -->
        <v-text-field v-model="form.type_of_validity" label="Тип валидности" />

        <!-- Ответственное лицо -->
        <v-text-field v-model="form.responsible_person_id" label="ID ответственного лица" />
...
const props = defineProps(['modelValue', 'contract'])
